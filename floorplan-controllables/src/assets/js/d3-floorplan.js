console.log("Loading d3 floorplan")
    ...
    function a(a) {
        ...
        a.each(function(a) {
            if (a) {
                ...
                b = b.select("g.layer-controls").selectAll("g").data(e, function(a) { return a.id() });
                ...
                g.append("text").attr("x", 10).attr("y", 15).style("font-size", "12px").style("font-family", "Helvetica, Arial, sans-serif").text(function(a) { return a.title() });
                b.transition().duration(1E3).attr("transform", function(a, b) { return "translate(0," + 20 * (e.length - (b + 1)) + ")" });
                c = c.selectAll(".maplayer").data(e, function(a) { return a.id() });
                c.enter().append("g").attr("class", function(a) {
                    return "maplayer " +
                        a.title()
                }).append("g").attr("class", function(a) { return a.id() }).datum(null);
                c.exit().remove();
                c.order();
...
d3.floorplan.imagelayer = function() {
    function a(a) {
        a.each(function(a) {
            a && (a = d3.select(this).selectAll("image").data(a, function(a) { return a.url }), a.enter().append("image").attr("xlink:href", function(a) { return a.url }).style("opacity", 1E-6), a.exit().transition().style("opacity", 1E-6).remove(), a.transition().attr("x", function(a) { return e(a.x) }).attr("y", function(a) { return f(a.y) }).attr("height", function(a) { return f(a.y + a.height) - f(a.y) }).attr("width", function(a) { return e(a.x + a.width) - e(a.x) }).style("opacity",
                function(a) { return a.opacity || 1 }))
        })
    }
...
d3.floorplan.heatmap = function() {
    function a(a) {
        a.each(function(a) {
            if (a && a.map) {
                ...
                var b = a.map.map(function(a) { return a.value }).sort(d3.ascending),
                    g, j;
                switch (f) {
                    case "quantile":
                    ...
                    case "normal":
                        var m =
                            d3.mean(b),
                            b = Math.sqrt(d3.sum(b, function(a) { return Math.pow(a - m, 2) }) / b.length);
                ...
                d.classed(e, !0);
                this.__colors__ = e;
                b = d.selectAll("rect").data(a.map.filter(function(a) { return !a.points }), function(a) { return a.x + "," + a.y });
                j = b.enter().append("rect").style("opacity", 1E-6);
                b.exit().transition().style("opacity", 1E-6).remove();
                j.append("title");
                b.attr("x", function(a) { return i(a.x) }).attr("y", function(a) { return h(a.y) }).attr("height", Math.abs(h(a.binSize) - h(0))).attr("width", Math.abs(i(a.binSize) - i(0))).attr("class", function(a) { return "d6-" + g(a.value) }).select("title").text(function(b) {
                    return "value: " +
                        c(b.value) + a.units
                });
                j.transition().style("opacity", 0.6);
                b = d.selectAll("path").data(a.map.filter(function(a) { return a.points }), function(a) { return JSON.stringify(a.points) });
                j = b.enter().append("path").attr("d", function(a) { return l(a.points) + "Z" }).style("opacity", 1E-6);
                b.exit().transition().style("opacity", 1E-6).remove();
                j.append("title");
                b.attr("class", function(a) { return "d6-" + g(a.value) }).select("title").text(function(b) { return "value: " + c(b.value) + a.units });
                j.transition().style("opacity", 0.6);
                d = d.selectAll("text").data(a.map.filter(function(a) { return a.points }),
                    function(a) { return JSON.stringify(a.points) });
                ...
                }).text(function(b) { return c(b.value) + a.units });
    ...
    var e = "RdYlBu",
        ...
        i = d3.scale.linear(),
        h = d3.scale.linear(),
        l = d3.svg.line().x(function(a) { return i(a.x) }).y(function(a) { return h(a.y) }),
        c = d3.format(".4n"),
...
d3.floorplan.overlays = function() {
        ...
        a.each(function(a) {
            if (a) {
                ...
                g.enter().append("rect").attr("class", "overlay-canvas").style("opacity", 0).attr("pointer-events", "all").on("click", function() {
                    if (j) {
                        var a = d3.mouse(this);
                        c.forEach(function(b) { b(f.invert(a[0]), i.invert(a[1])) })
                    }
                ...
                g.attr("x", f.range()[0]).attr("y", i.range()[0]).attr("height", i.range()[1] - i.range()[0]).attr("width", f.range()[1] -
                    f.range()[0]);
                g = b.selectAll("path.polygon").data(a.polygons || [], function(a) { return a.id });
                ...
                g.attr("d", function(a) { return k(a.points) + "Z" }).style("cursor", j ? "move" : "pointer").select("title").text(function(a) { return a.name || a.id });
                if (j) {
                    var h = [];
                    a.polygons && a.polygons.forEach(function(a) { a.points.forEach(function(b, d) { h.push({ index: d, parent: a }) }) });
                    a = 1;
                    for (g = b.node(); g.parentNode;)
                        if (g = g.parentNode, g.__scale__) { a = g.__scale__; break }
                    b = b.selectAll("circle.vertex").data(h, function(a) { return a.parent.id + "-" + a.index });
                    ...
                    b.attr("cx", function(a) { return f(a.parent.points[a.index].x) }).attr("cy",
                        function(a) { return i(a.parent.points[a.index].y) }).attr("r", 4 / a)
    ...
    var f = d3.scale.linear(),
        ...
        n = [],
        j = !1,
        k = d3.svg.line().x(function(a) { return f(a.x) }).y(function(a) { return i(a.y) }),
    ...
    a.registerCanvasCallback = function(b) { arguments.length && c.push(b); return a };
    a.registerSelectCallback = function(b) { arguments.length && select.Callbacks.push(b); return a };
    a.registerMoveCallback = function(b) { arguments.length && n.push(b); return a };
    return a
};
d3.floorplan.vectorfield = function() {
    function a(a) {
        a.each(function(a) {
            if (a && a.map) {
                var c = d3.select(this).selectAll("path.vector").data(a.map, function(a) { return a.x + "," + a.y });
                ...
                var e = a.binSize / 2 / d3.max(a.map, function(a) { return Math.max(Math.abs(a.value.x), Math.abs(a.value.y)) });
                c.attr("d", function(c) {
                    var f = {
                        x: c.x + a.binSize /
                            2,
                ...
                }).select("title").text(function(c) { return Math.sqrt(c.value.x * c.value.x + c.value.y * c.value.y) + " " + a.units });
    ...
    var e = d3.scale.linear(),
        f = d3.scale.linear(),
        i = d3.svg.line().x(function(a) { return e(a.x) }).y(function(a) { return f(a.y) }),
...
d3.floorplan.pathplot = function() {
    function a(a) { a.each(function(a) { a && (a = d3.select(this).selectAll("path").data(a, function(a) { return a.id }), a.exit().transition().style("opacity", 1E-6).remove(), a.enter().append("path").attr("vector-effect", "non-scaling-stroke").attr("fill", "none").style("opacity", 1E-6).append("title"), a.attr("class", function(a) { return a.classes || a.id }).attr("d", function(a, d) { return i(c(a, d)) }).select("title").text(function(a) { return a.title || a.id }), a.transition().style("opacity", 1)) }) }
    var e = d3.scale.linear(),
        f = d3.scale.linear(),
        i = d3.svg.line().x(function(a) { return e(a.x) }).y(function(a) { return f(a.y) }),
        h = "fp-pathplot-" + (new Date).valueOf(),
        l = "pathplot",
        c = function(a) { return a.points };
    a.xScale = function(c) {
        if (!arguments.length) return e;
        e = c;
        return a
